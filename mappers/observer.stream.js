...
const Mapper = function (OBJY, options) {
    ...
    this.getEvent = function (objId, propName, success, error, client) {
        ...
        if (!this.index[client][objId + ':' + propName]) return error('object not found: ' + objId + ':' + propName);
    ...
    this.addEvent = function (objId, propName, event, success, error, client) {
        ...
        if (this.index[client][objId + ':' + propName]) return error('object with that id already exists: ' + objId);

        if (!this.index[client]) this.index[client] = {};

        var db = this.getDBByMultitenancy(client);

        if (this.multitenancy == CONSTANTS.MULTITENANCY.ISOLATED) event.tenantId = client;
        ...
        if (event.date) {
            ...
            db.push(
                setTimeout(function () {
                    ...
                    if (self.OBJY.processors[self.objectFamily])
