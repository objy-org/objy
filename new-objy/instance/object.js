...
export default function (OBJY) {
    return {
        ...
        getTemplateFieldsForObject: function (obj, templateId, success, error, client, templateRole, templateSource, params, instance) {
                ...
                function doTheProps(template, obj) {
                    ...
                    if (params.extendedStructure) {
                        ...
                        Object.keys(template).forEach(function(p) {
                            if(!OBJY.instance.predefinedProperties.includes(p) && isObject(template[p])){
                    ...
                    Object.keys(template).forEach((p) => {
                        ...
                        if (!obj.hasOwnProperty(p) && typeof cloned === 'object') {
            ...
            if (this.instance.caches[templateRole || obj.role].get(templateId)) {
                //run(this.instance.caches[templateRole || obj.role].get(templateId))
            } else {
                OBJY.getObjectById(
